...
In addition to the data processing scripts, the repository contains scripts that generate and post daily updates to an account on Twitter, [@RedSoxDataBot](https://x.com/RedSoxDataBot).
...
- **News roundup**: The `scripts/24_fetch_news.py` script fetches the top Red Sox-related headlines. It then formats these into a single tweet. This is automated by the `.github/workflows/post_news.yml` workflow.
...
- **`post_summaries.yml`**: Posts statistical summaries to Twitter.
- **`tweet_lineup.yml`**: Checks hourly for the day's lineup and posts the pitching matchup to Twitter once available.
- **`post_news.yml`**: Fetches and posts a news roundup to Twitter.
...
### Twitter Secrets (Optional - Currently Disabled)

If you want to re-enable Twitter functionality in the future:

| Secret Name | Description | How to Obtain |
|------------|-------------|---------------|
| `TWITTER_API_KEY` | Twitter API v2 Consumer Key | [Twitter Developer Portal](https://developer.twitter.com/) → Create App |
| `TWITTER_API_SECRET` | Twitter API v2 Consumer Secret | Same as above |
| `TWITTER_ACCESS_TOKEN` | User access token | Generate in Twitter App settings |
| `TWITTER_ACCESS_SECRET` | User access token secret | Same as above |
...
The project runs on these automated schedules (all times UTC):

| Workflow | Schedule | Purpose |
|----------|----------|---------|
| `fetch.yml` | Every 3 hours during season | Fetch latest stats and rebuild data |
| `build_site.yml` | Daily at 12:00 UTC | Rebuild Jekyll site with latest data |
| `post_summaries.yml` | Disabled | Twitter summaries (re-enable if needed) |
| `tweet_lineup.yml` | Disabled | Daily lineup tweets (re-enable if needed) |
| `post_news.yml` | Disabled | News roundup tweets (re-enable if needed) |

**Note:** Twitter workflows are currently disabled. To re-enable, uncomment the schedule triggers in the respective YAML files.
